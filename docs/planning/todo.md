...
- B-012 (P1, MITIGATING): Direction/dynamics/text lane collisions are substantially reduced. Current generalized mitigations include top-staff-only default routing for unstaffed directions, per-system lane persistence (directions/harmonies/lyrics), interval-based row occupancy routing, expanded direction-lane vertical clearance, dynamics/words dedupe when both encode the same marking, style-accurate harmony width measurement with side-padding row packing, compact harmony-kind formatting (for readable chord symbols), and text-pressure-aware automatic inter-system gap expansion that discounts dynamics-only pressure. Latest snapshots: `31a` text overlaps `2`, `31d` text overlaps `1` (from `8`), `71f` text overlaps `0` (from `10`), `61b` text overlaps `0`, `32a` text overlaps `3`, and `31a` dynamics-to-text overlaps `2`. Regression budgets were tightened accordingly (`31a<=2`, `31d<=2`, `71f<=1`, `32a<=4`, `31a dynamics/text<=2`); keep MITIGATING until these tighter gates remain stable across the wider proof-point set.
